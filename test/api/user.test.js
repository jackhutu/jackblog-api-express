...
describe('test/api/user.test.js',function () {
  var token,mockUserId,mockAdminId,mockNickname,mockUpdateNickName,mockAdminNickname;
  before(function (done) {
    authHelper.createUser('admin').then(function (user) {
      mockAdminId = user._id;
      mockAdminNickname = user.nickname;
      return user;
    }).then(function (user) {
  ...
  describe('put /users/:id/updateUser', function() {
    ...
    it('should update user same nickname return error', function(done) {
      request.put('/users/' + mockUserId + '/updateUser')
      .set('Authorization','Bearer ' + token)
      .send({
